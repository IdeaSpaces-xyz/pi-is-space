...
The tool roster is fixed at session start: every `is_*` tool is registered at load with its schema serialized in sorted key order, the roster is sealed, and nothing joins or leaves afterwards — tools render first in the provider prompt, so a roster change would invalidate every cached byte behind it. Tool closure never removes; release applies to content, not the roster.
