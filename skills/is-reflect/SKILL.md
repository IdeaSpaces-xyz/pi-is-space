...
Verify before claiming status — the Delta Protocol in [awareness](../../reference/awareness.md). In short: check the doc against the code (which may live in a sibling repo) with `bash` (`rg`, `git log`, `git diff`) and `read`; doc status lines are hints, not authority.
