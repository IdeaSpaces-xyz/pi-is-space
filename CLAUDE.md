...
On session start, assemble local awareness in-process from `@ideaspaces/protocol`: structured Content awareness supplies the composed contract, position, tree/context summaries, drift, and recent activity; its placement renderer supplies the stable `head` and volatile `tail` without Pi owning a section list. Protocol workspace/root handles project through Map members and the shared renderer; protocol git/path primitives supply capture state. Pi owns only the kind line (the convention a Space's Agreement declares, after the protocol head — the same line the Claude Code plugin renders), the working-set/catalog roles and state, the split cache placement (stable register in the system prompt via `before_agent_start`; volatile register appended after the last cache breakpoint via `before_provider_request` — design and measurement in `pi-cache-placement` in the team roadmap space, a separate private repo), the richer State block that explicitly replaces compact protocol Git text, the Change line, and the seen-ref write. Missing `_agent/purpose.md` / `_agent/now.md` are drift signals, not placeholders to silently fill.
